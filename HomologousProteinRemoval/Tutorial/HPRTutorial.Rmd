...
In your dataset, there are only two required variables (though you can have more). The first is a "Protein name" variable that has some pattern to identify the genus and species. For example, in the screenshot of the dataset below the genus and species are identifiable after "OS=". This gives us some way to search for and extract the genus and species name. The second variable your dataset must containis the grouping variable. This will be the variable the function will look to see if there is ambiguity within each group.
